...
function computeReadiness(student) {
  ...
  const recentTests = db.raw.dailyTests
    .filter((t) => t.studentId === student.id)
    .sort((a, b) => b.takenAt - a.takenAt)
    .slice(0, minRecentTests);
  ...
  const avgAccuracy = recentTests.length
    ...
    : 0;
  const streakOk = (student.streak || 0) >= minStreakDays;
...
function studentSummary(s) {
  ...
  const readiness = isMaster ? null : computeReadiness(s);
  return {
...
route('POST', '/api/daily-test/start', requireStudent(async (req, res, params, body, student) => {
  ...
  const generated = generateProblems(tables, DAILY_TEST_CONFIG.questionCount);
...
route('POST', '/api/level-exam/start', requireStudent(async (req, res, params, body, student) => {
  ...
  const readiness = computeReadiness(student);
  if (!readiness.eligible) {
    return sendJson(res, 403, { error: '아직 승급 시험 자격 기준을 채우지 못했어요. 오늘의 테스트를 꾸준히 풀어주세요.', readiness });
  }
  const levelDef = getLevelDef(student.level);
  const generated = generateLevelExamProblems(levelDef);
  const { examId, problems } = exam.start({
    studentId: student.id,
    kind: 'level',
    level: student.level,
  ...
  sendJson(res, 200, { examId, problems, config: levelDef.examConfig, levelDef });
}));
...
route('POST', '/api/level-exam/submit', requireStudent(async (req, res, params, body, student) => {
  ...
  const examConfig = pendingExam.config || getLevelDef(student.level).examConfig;
  const elapsed = Number(elapsedSec) || 0;
  const timedOut = elapsed > examConfig.timeLimitSec + examConfig.graceSec;
  const passed = score >= examConfig.passScore && !timedOut;
